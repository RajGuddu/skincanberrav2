...
class Testimonial extends Controller
{
    ...
    public function index(Request $request){
        ...
        $data['testiList'] = $this->commonmodel->crudOperation('RA','tbl_testimonial','','',['id','DESC']);
        ...
        return view('admin.testimonial.testiIndex', $data);
    }
    public function add_edit_testimonial(Request $request, $id=null){
        $data = $post = [];
        if($request->isMethod('POST')){
            ...
            if($validated){
                if($request->hasFile('photo')){
                    if ($request->file('photo')->isValid()) {
                        ...
                        do {
                            ...
                            $exists = $this->commonmodel->isExists('tbl_testimonial',['photo'=>$webpFilename]);
                        } while ($exists);
                ...
                if(!$id){
                    ...
                    $inserted = $this->commonmodel->crudOperation('C','tbl_testimonial',$post);

                }else{
                    $post['update_at'] = date('Y-m-d H:i:s');
                    $updated = $this->commonmodel->crudOperation('U','tbl_testimonial',$post,['id'=>$id]);
                }
                if(isset($inserted)){
        ...
        if($id){
            $data['record'] = $this->commonmodel->crudOperation('R1','tbl_testimonial','',['id'=>$id]);
        }
        return view('admin.testimonial.add_edit_testimonial', $data);
    }
    public function delete_testimonial(Request $request, $id=null){
        if($id){
            $record = $this->commonmodel->crudOperation('R1','tbl_testimonial','',['id'=>$id]);
            if(!empty($record)){
                ...
                if($this->commonmodel->crudOperation('D','tbl_testimonial','',['id'=>$id])){
