...
<div class="app-content pt-3 p-md-3 p-lg-4">
	<div class="container-fluid">
		<div class="d-flex align-items-center justify-content-between">
			<h1 class="app-page-title">Add Testimonials</h1>
			<div class="">
				<a class="btn app-btn-secondary" href="{{ url('admin/testimonials') }}"> Back </a>
			</div>
		</div>
		<hr class="mb-4">
		<div class="row g-4">
						...
						<form class="" action="{{ url()->current() }}" method="post" enctype="multipart/form-data">
							...
							<div class="mb-3">
								...
								<input type="text" class="form-control" id="post" name="post"
									value="{{ old('post', $record->post ?? '') }}">
								@error('post') <span class="text-danger"> {{ $message }} </span> @enderror
							</div>
							<div class="mb-3">
								<label for="photo" class="form-label">Photo(452 X 208 px)</label>
								<input type="file" class="form-control" id="photo" name="photo">
								<input type="hidden" name="photo2" value="{{ $record->photo ?? '' }}">
							</div>
							<?php /* <div class="mb-3">
							...
							<div class="mb-3">
								<label for="description" class="form-label">Details (less than 250 character)</label>
							...
							<div class="mb-3">
								<label for="thumb_image" class="form-label">Thumbnail Image(452 X 208 px)</label>
							...
							<a href="{{ url('admin/testimonials') }}" class="btn app-btn-secondary">Cancel</a>
						</form>
					</div><!--//app-card-body-->
						...
						<div class="d-flex align-content-start flex-wrap my-2">
							...
							<div class="img-box">
								<span class="cancel-icon"
									onclick="cancel_image_('tbl_testimonial','photo','id', <?=$record->id?>)"><i
										class="fa-solid fa-xmark" title="Cancel"></i>
								</span>
								<img src="{{ url(IMAGE_PATH.$record->photo) }}" class="" alt="...">
								<small class="image-title">Photo</small>
							</div>
							@endif
							@if(isset($record) && $record->thumb_image != '')
							@php $isImage = 1; @endphp
							<div class="img-box">
								<span class="cancel-icon"
